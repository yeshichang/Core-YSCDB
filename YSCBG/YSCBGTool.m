...
#define YSCBGValue @"YSCBGValue"
#define YSCBGData @"YSCBGData"
#define YSCBGArray @"YSCBGArray"
#define YSCBGSet @"YSCBGSet"
#define YSCBGDictionary @"YSCBGDictionary"
#define YSCBGModel @"YSCBGModel"
#define YSCBGMapTable @"YSCBGMapTable"
#define YSCBGHashTable @"YSCBGHashTable"
 ...
 *  遍历所有类的block（父类）
 */
typedef void (^YSCBGClassesEnumeration)(Class c, BOOL *stop);
static NSSet *foundationClasses_;

@implementation YSCBGTool
/**
 封装处理传入数据库的key和value.
 */
NSString* YSCBG_sqlKey(NSString* key){
    return [NSString stringWithFormat:@"%@%@",YSCBG,key];
}

/**
 转换OC对象成数据库数据.
 */
NSString* YSCBG_sqlValue(id value){
    ...
    value = [YSCBGTool getSqlValue:value type:type encode:YES];
 ...
 */
NSString* YSCBG_keyPathValues(NSArray* keyPathValues){
    return [YSCBGTool getLikeWithKeyPathAndValues:keyPathValues where:NO];
}
/**
 自定义数据库名称.
 */
void YSCBG_setSqliteName(NSString*_Nonnull sqliteName){
    if (![sqliteName isEqualToString:[YSCBGDB shareManager].sqliteName]) {
        [YSCBGDB shareManager].sqliteName = sqliteName;
    }
}
/**
 删除数据库文件
 */
BOOL YSCBG_deleteSqlite(NSString*_Nonnull sqliteName){
    return [YSCBGDB deleteSqlite:sqliteName];
}
/**
 设置操作过程中不可关闭数据库(即closeDB函数无效).
 默认是NO.
 */
void YSCBG_setDisableCloseDB(BOOL disableCloseDB){
    if ([YSCBGDB shareManager].disableCloseDB != disableCloseDB){//防止重复设置.
        [YSCBGDB shareManager].disableCloseDB = disableCloseDB;
    }
}
/**
 设置调试模式
 ...
 */
void YSCBG_setDebug(BOOL debug){
    if ([YSCBGDB shareManager].debug != debug){//防止重复设置.
        [YSCBGDB shareManager].debug = debug;
    }
}
 ...
 */
void YSCBG_inTransaction(BOOL (^ _Nonnull block)(void)){
    [[YSCBGDB shareManager] inTransaction:block];
}
/**
 清除缓存
 */
void YSCBG_cleanCache(){
    [[NSCache YSCBG_cache] removeAllObjects];
}
/**
 json字符转json格式数据 .
 */
...
+ (void)YSCBG_enumerateClasses:(__unsafe_unretained Class)srcCla complete:(YSCBGClassesEnumeration)enumeration
{
    // 1.没有block就直接返回
...
+(NSArray*)getClassIvarList:(__unsafe_unretained Class)cla Object:(_Nullable id)object onlyKey:(BOOL)onlyKey{
    ...
    if(onlyKey){
        [keys addObject:YSCBG_primaryKey];
        [keys addObject:YSCBG_createTimeKey];
        [keys addObject:YSCBG_updateTimeKey];
    }else{
        //手动添加库自带的自动增长主键ID和类型q
        [keys addObject:[NSString stringWithFormat:@"%@*q",YSCBG_primaryKey]];
        //建表时此处加入额外的两个字段(createTime和updateTime).
        [keys addObject:[NSString stringWithFormat:@"%@*@\"NSString\"",YSCBG_createTimeKey]];
        [keys addObject:[NSString stringWithFormat:@"%@*@\"NSString\"",YSCBG_updateTimeKey]];
    }
    //判断是否有自定义类型标志，有则不缓存.
    __block BOOL isKindOfCustomFlag = NO;
    [self YSCBG_enumerateClasses:cla complete:^(__unsafe_unretained Class c, BOOL *stop) {
...
+(NSArray*)where:(NSArray*)where{
    ...
    if(!(where.count%3)){
        ...
        for(int i=0;i<where.count;i+=3){
            [SQL appendFormat:@"%@%@%@?",YSCBG,where[i],where[i+1]];
            if (i != (where.count-3)) {
...
+(NSString*)getLikeWithKeyPathAndValues:(NSArray* _Nonnull)keyPathValues where:(BOOL)where{
    ...
    for(int i=0;i<keys.count;i++){
        ...
        if ([relations[i] isEqualToString:YSCBG_contains]){//包含关系
            [keyPathParam appendString:@"%"];
        }else{
            if(keypaths.count<=2){
        ...
        [likeM appendFormat:@"%@%@ like '%@'",YSCBG,keypaths[0],keyPathParam];
        if(i != (keys.count-1)){
...
+(NSString*)keyAndType:(NSString*)param{
    ...
    return [NSString stringWithFormat:@"%@ %@",[NSString stringWithFormat:@"%@%@",YSCBG,key],SqlType];
}
...
+(NSString *)jsonStringWithObject:(id)object{
    ...
    for(NSString* keyAndType in keyAndTypes){
        ...
        if(![propertyName isEqualToString:YSCBG_primaryKey]){
...
+(NSDictionary*)dictionaryForArrayInsert:(id)value{
    ...
    if ([value isKindOfClass:[NSArray class]]){
        return @{YSCBGArray:[self jsonStringWithArray:value]};
    }else if ([value isKindOfClass:[NSSet class]]){
        return @{YSCBGSet:[self jsonStringWithArray:value]};
    }else if ([value isKindOfClass:[NSString class]] || [value isKindOfClass:[NSNumber class]]){
        return @{YSCBGValue:value};
    }else if([value isKindOfClass:[NSData class]]){
        NSData* data = value;
        NSNumber* maxLength = MaxData;
        NSAssert(data.length<maxLength.integerValue,@"最大存储限制为100M");
        return @{YSCBGData:[value base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength]};
    }else if ([value isKindOfClass:[NSDictionary class]]){
        return @{YSCBGDictionary:[self jsonStringWithDictionary:value]};
    }else if ([value isKindOfClass:[NSMapTable class]]){
        return @{YSCBGMapTable:[self jsonStringWithMapTable:value]};
    }else if([value isKindOfClass:[NSHashTable class]]){
        return @{YSCBGHashTable:[self jsonStringWithNSHashTable:value]};
    }else{
        NSString* modelKey = [NSString stringWithFormat:@"%@*%@",YSCBGModel,NSStringFromClass([value class])];
...
+(NSString *)getTableNameWithObject:(id)object{
    NSString* tablename = [object valueForKey:YSCBG_tableNameKey];
    if(tablename == nil) {
...
+(id)objectFromJsonStringWithTableName:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla valueDict:(NSDictionary*)valueDict{
    ...
    for(int i=0;i<valueDictKeys.count;i++){
        ...
        if([sqlKey containsString:YSCBG]){
            tempSqlKey = [sqlKey stringByReplacingOccurrencesOfString:YSCBG withString:@""];
        }
        for(NSString* keyAndType in keyAndTypes){
            ...
            if ([tempSqlKey isEqualToString:key]){
                ...
                if([tempValue isKindOfClass:[NSString class]] && [tempValue containsString:YSCBG_CUSTOM_TYPE_SEPARATOR]){
                    NSArray* arrTempTV = [tempValue componentsSeparatedByString:YSCBG_CUSTOM_TYPE_SEPARATOR];
    ...
    [object setValue:tablename forKey:YSCBG_tableNameKey];
 ...
 */
+(id)YSCBG_objectWithClass:(__unsafe_unretained _Nonnull Class)cla value:(id)value{
    ...
    NSDictionary* const objectClaInArr = [YSCBGTool executeSelector:NSSelectorFromString(@"YSCBG_objectClassInArray") forClass:[object class]];
    NSDictionary* const objectClaForCustom = [YSCBGTool executeSelector:NSSelectorFromString(@"YSCBG_objectClassForCustom") forClass:[object class]];
    NSDictionary* const YSCBG_replacedKeyFromPropertyNameDict = [YSCBGTool executeSelector:NSSelectorFromString(@"YSCBG_replacedKeyFromPropertyName") forClass:[object class]];
    ...
    //处理要替换的key和属性名.
    if(YSCBG_replacedKeyFromPropertyNameDict && YSCBG_replacedKeyFromPropertyNameDict.count){
        [YSCBG_replacedKeyFromPropertyNameDict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            ...
            if ([key isEqualToString:claKey]){
                ...
                !objectClaInArr?:[objectClaInArr enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull Arrkey, id  _Nonnull ArrObjCla, BOOL * _Nonnull stop){
                    if([key isEqualToString:Arrkey]){
                        ...
                        for(NSDictionary* ArrObj in dataDictObj){
                            id obj = [self YSCBG_objectWithClass:ArrObjCla value:ArrObj];
                ...
                !objectClaForCustom?:[objectClaForCustom enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull customKey, id  _Nonnull customObj, BOOL * _Nonnull stop) {
                    ...
                    if([key isEqualToString:tempKey]){
                        ArrObject = [self YSCBG_objectWithClass:customObj value:[dataDict valueForKey:tempKey]];
                        *stop = YES;
                    }
                }];
...
/**
 模型转字典.
 */
+(NSMutableDictionary*)YSCBG_keyValuesWithObject:(id)object ignoredKeys:(NSArray*)ignoredKeys{
    ...
    NSDictionary* const objectClaInArr = [YSCBGTool executeSelector:NSSelectorFromString(@"YSCBG_objectClassInArray") forClass:[object class]];
    NSDictionary* const objectClaForCustom = [YSCBGTool executeSelector:NSSelectorFromString(@"YSCBG_dictForCustomClass") forClass:[object class]];
    NSMutableDictionary* dictM = [NSMutableDictionary dictionary];
    ...
    [keys enumerateObjectsUsingBlock:^(NSString * _Nonnull key, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        !objectClaInArr?:[objectClaInArr enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull Arrkey, id  _Nonnull ArrObjCla, BOOL * _Nonnull stop){
            if([key isEqualToString:Arrkey]){
                ...
                for(id arrObj in value){
                    id dictObj = [self YSCBG_keyValuesWithObject:arrObj ignoredKeys:nil];
        ...
        !objectClaForCustom?:[objectClaForCustom enumerateKeysAndObjectsUsingBlock:^(NSString*  _Nonnull customKey, id  _Nonnull customObj, BOOL * _Nonnull stop) {
            if([key isEqualToString:customKey]){
                value = [self YSCBG_keyValuesWithObject:[object valueForKey:customKey] ignoredKeys:nil];
                *stop = YES;
            }
        }];
...
+(id)valueForArrayRead:(NSDictionary*)dictionary{
    ...
    if ([key isEqualToString:YSCBGValue]) {
        return dictionary[key];
    }else if ([key isEqualToString:YSCBGData]){
       return [[NSData alloc] initWithBase64EncodedString:dictionary[key] options:NSDataBase64DecodingIgnoreUnknownCharacters];
    }else if([key isEqualToString:YSCBGSet]){
        return [self arrayFromJsonString:dictionary[key]];
    }else if([key isEqualToString:YSCBGArray]){
        return [self arrayFromJsonString:dictionary[key]];
    }else if ([key isEqualToString:YSCBGDictionary]){
       return [self dictionaryFromJsonString:dictionary[key]];
    }else if ([key containsString:YSCBGModel]){
...
+(NSArray*)arrayFromJsonString:(NSString*)jsonString{
    ...
    if([jsonString containsString:YSCBGModel] || [jsonString containsString:YSCBGData]){
        NSMutableArray* arrM = [NSMutableArray array];
...
+(id)valueForDictionaryRead:(NSDictionary*)dictDest{
    ...
    if([keyDest isEqualToString:YSCBGValue]){
        return dictDest[keyDest];
    }else if ([keyDest isEqualToString:YSCBGData]){
        return [[NSData alloc] initWithBase64EncodedString:dictDest[keyDest] options:NSDataBase64DecodingIgnoreUnknownCharacters];
    }else if([keyDest isEqualToString:YSCBGSet]){
        return [self arrayFromJsonString:dictDest[keyDest]];
    }else if([keyDest isEqualToString:YSCBGArray]){
        return [self arrayFromJsonString:dictDest[keyDest]];
    }else if([keyDest isEqualToString:YSCBGDictionary]){
        return [self dictionaryFromJsonString:dictDest[keyDest]];
    }else if([keyDest containsString:YSCBGModel]){
...
+(NSArray*)tansformDataFromSqlDataWithTableName:(NSString*)tableName class:(__unsafe_unretained _Nonnull Class)cla array:(NSArray*)array{
    ...
    NSMutableArray* arrM = [NSMutableArray array];
    for(NSDictionary* dict in array){
        id object = [YSCBGTool objectFromJsonStringWithTableName:tableName class:cla valueDict:dict];
...
+(NSDictionary* _Nonnull)getDictWithObject:(id _Nonnull)object ignoredKeys:(NSArray* const _Nullable)ignoredKeys filtModelInfoType:(YSCBG_getModelInfoType)filtModelInfoType{
    NSArray<YSCBGModelInfo*>* infos = [YSCBGModelInfo modelInfoWithObject:object];
    NSMutableDictionary* valueDict = [NSMutableDictionary dictionary];
    if (ignoredKeys) {
        for(YSCBGModelInfo* info in infos){
    ...
    if (filtModelInfoType == YSCBG_ModelInfoSingleUpdate){//单条更新操作时,移除 创建时间和主键 字段不做更新
        [valueDict removeObjectForKey:YSCBG_sqlKey(YSCBG_createTimeKey)];
        //判断是否定义了“联合主键”.
        NSArray* unionPrimaryKeys = [YSCBGTool executeSelector:YSCBG_unionPrimaryKeysSelector forClass:[object class]];
        NSString* YSCBG_id = YSCBG_sqlKey(YSCBG_primaryKey);
        if(unionPrimaryKeys.count == 0){
            if([valueDict.allKeys containsObject:YSCBG_id]) {
                [valueDict removeObjectForKey:YSCBG_id];
            }
        }else{
            if(![valueDict.allKeys containsObject:YSCBG_id]) {
                valueDict[YSCBG_id] = @(1);//没有就预备放入
            }
        }
    }else if(filtModelInfoType == YSCBG_ModelInfoInsert){//插入时要移除主键,不然会出错.
        //判断是否定义了“联合主键”.
        NSArray* unionPrimaryKeys = [YSCBGTool executeSelector:YSCBG_unionPrimaryKeysSelector forClass:[object class]];
        NSString* YSCBG_id = YSCBG_sqlKey(YSCBG_primaryKey);
        if(unionPrimaryKeys.count == 0){
    ...
    }else if(filtModelInfoType == YSCBG_ModelInfoArrayUpdate){//批量更新操作时,移除 创建时间 字段不做更新
        [valueDict removeObjectForKey:YSCBG_sqlKey(YSCBG_createTimeKey)];
    }else;
...
/**
 过滤建表的key.
 */
+(NSArray*)YSCBG_filtCreateKeys:(NSArray*)YSCBG_createkeys ignoredkeys:(NSArray*)YSCBG_ignoredkeys{
    NSMutableArray* createKeys = [NSMutableArray arrayWithArray:YSCBG_createkeys];
    NSMutableArray* ignoredKeys = [NSMutableArray arrayWithArray:YSCBG_ignoredkeys];
    //判断是否有需要忽略的key集合.
...
+(BOOL)ifNotExistWillCreateTableWithObject:(id)object ignoredKeys:(NSArray* const)ignoredKeys{
    ...
    //获取"唯一约束"字段名
    NSArray* uniqueKeys = [YSCBGTool executeSelector:YSCBG_uniqueKeysSelector forClass:[object class]];
    //获取“联合主键”字段名
    NSArray* unionPrimaryKeys = [YSCBGTool executeSelector:YSCBG_unionPrimaryKeysSelector forClass:[object class]];
    __block BOOL isExistTable;
    [[YSCBGDB shareManager] isExistWithTableName:tableName complete:^(BOOL isExist) {
        if (!isExist){//如果不存在就新建
            NSArray* createKeys = [self YSCBG_filtCreateKeys:[YSCBGTool getClassIvarList:[object class] Object:object onlyKey:NO] ignoredkeys:ignoredKeys];
            [[YSCBGDB shareManager] createTableWithTableName:tableName keys:createKeys unionPrimaryKeys:unionPrimaryKeys uniqueKeys:uniqueKeys complete:^(BOOL isSuccess) {
