...
+ (instancetype)resultSetWithStatement:(YSCStatement *)statement usingParentDatabase:(YSCDatabase*)aDB {
    ...
    return YSCDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    YSCDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return YSCDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"YSCDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
