...
#import "YSCResultSet.h"
#import "YSCDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define YSCDBAutorelease(__v) ([__v autorelease]);
    #define YSCDBReturnAutoreleased YSCDBAutorelease

    #define YSCDBRetain(__v) ([__v retain]);
    #define YSCDBReturnRetained YSCDBRetain

    #define YSCDBRelease(__v) ([__v release]);

    #define YSCDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define YSCDBAutorelease(__v)
    #define YSCDBReturnAutoreleased(__v) (__v)

    #define YSCDBRetain(__v)
    #define YSCDBReturnRetained(__v) (__v)

    #define YSCDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define YSCDBDispatchQueueRelease(__v)
    #else
        #define YSCDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^YSCDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, YSCDBCheckpointMode) {
    YSCDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    YSCDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    YSCDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    YSCDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in YSCDB are:

 - `YSCDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<YSCResultSet>` - Represents the results of executing a query on an `YSCDatabase`.
 - `<YSCDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<YSCDatabasePool>` - A pool of `YSCDatabase` objects.
 - `<YSCStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [YSCDB on GitHub](https://github.com/ccgus/YSCdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [YSCDB mailing list](http://groups.google.com/group/YSCdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `YSCDatabase` object and use it across multiple threads. Instead, use `<YSCDatabaseQueue>`.
...
@interface YSCDatabase : NSObject
...
/** Create a `YSCDatabase` object.
 
 An `YSCDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `YSCDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `YSCDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    YSCDatabase *db = [YSCDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    YSCDatabase *db     = [YSCDatabase databaseWithPath:dbPath];
 ...
 @return `YSCDatabase` object if successful; `nil` if failure.
...
/** Create a `YSCDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `YSCDatabase` connection is closed.
...
/** Initialize a `YSCDatabase` object.
...
/** Initialize a `YSCDatabase` object.
 ...
 An `YSCDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) YSCDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<YSCResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[YSCResultSet next]>`) from one record to the other.
 ...
 @return A `<YSCResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see YSCResultSet
 @see [`YSCResultSet next`](<[YSCResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (YSCResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (YSCResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (YSCResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> YSCResultSet!`
...
- (YSCResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (YSCResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (YSCResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Begin a transaction
 ...
 @warning    Unlike SQLite's `BEGIN TRANSACTION`, this method currently performs
             ...
             is likely to change in future versions of YSCDB, whereby this method
 ...
 */
- (BOOL)checkpoint:(YSCDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(YSCDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(YSCDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    YSCResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [YSCDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one YSCDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using YSCDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to YSCDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using YSCDB you will not need to interact directly with `YSCStatement`, but rather with `<YSCDatabase>` and `<YSCResultSet>` only.
...
@interface YSCStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
