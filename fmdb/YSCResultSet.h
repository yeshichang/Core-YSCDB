...
/** Represents the results of executing a query on an `<YSCDatabase>`.
...
@interface YSCResultSet : NSObject

@property (nonatomic, retain, nullable) YSCDatabase *parentDB;
...
/** `YSCStatement` used by result set. */

@property (atomic, retain, nullable) YSCStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<YSCStatement>`
 
 @param statement A `<YSCStatement>` to be performed
 
 @param aDB A `<YSCDatabase>` to be used
 
 @return A `YSCResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(YSCStatement *)statement usingParentDatabase:(YSCDatabase*)aDB;
